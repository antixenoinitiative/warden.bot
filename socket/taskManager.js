            ...
            try {
                ...
                if (data.promotion.commandAsk == "promotion" && botIdent().activeBot.botName == "GuardianAI") {
                    try {
                        ...
                        const sql = `UPDATE promotion SET axiChallenge_state = 1, axi_rolesCheck = (?)  WHERE userId = (?);`
                    ...
                    if (axiRoles.includes(hasMatchingRole)) {
                        ...
                        data.commandChan.forEach(async chan => {
                            if (data.promotion.requestor_threadId == chan) {
                                ...
                                const values = [embedId.id,data.promotion.userId]
                                const sql = `UPDATE promotion SET axiChallenge_state = 1, requestor_roleEmbedId = (?) WHERE userId = (?);`
                                await database.query(sql, values)
                            }
                            if (data.promotion.leadership_threadId == chan) {
                                const embedId = await guild.channels.cache.get(chan).send({ embeds: [embed] })
                                const values = [embedId.id,data.promotion.userId]
                                const sql = `UPDATE promotion SET axiChallenge_state = 1, leadership_roleEmbedId = (?) WHERE userId = (?);`
                                await database.query(sql, values)
                            }
                        })
                    ...
                    //Role not detected
                        ...
                        const requestor_components = new Discord.ActionRowBuilder()
                            ...
                            if (data.promotion.requestor_threadId == chan) {
                                ...
                                const embedId = await guild.channels.cache.get(chan).send({ embeds: [embed] })
                                const values = [embedId.id,data.promotion.userId]
                                const sql = `UPDATE promotion SET axiChallenge_state = 0, leadership_roleEmbedId = (?), axi_rolesCheck = -2 WHERE userId = (?);`
                                await database.query(sql, values)
                            }
                        })
                        return
