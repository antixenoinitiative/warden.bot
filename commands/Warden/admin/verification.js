...
function buildChallengeOverviewPanel(verificationSettings, enabledChallengeIds, challengeId, challenge, components = []) {
    ...
    const fields = [
        ...
        { name: 'Questions', value: (effectiveChallenge.questions ?? []).map((question, index) => `${index + 1}. ${question.id} â€” ${question.label ?? 'Question'}`).join('\n') || 'None', inline: false },
...
function buildQuestionListResponse(challengeId, challenge, options = {}) {
    ...
    const fields = compact
        ? chunkQuestionListLines(questions.map((question, index) => `${index + 1}. ${question.id} â€” ${question.label ?? 'Question'} (${getQuestionTaskTypeLabel(getQuestionTaskType(question))})`))
...
function truncateSelectText(text, maxLength = 100) {
    ...
    return value.length <= maxLength ? value : `${value.slice(0, Math.max(0, maxLength - 1))}â€¦`;
}
...
async function handleQuestionClearModalSubmit(interaction, parts = []) {
    ...
    if (!paths) return respondAdminModalError(interaction, responseMode, { embeds: [userErrorEmbed('That clear action is no longer available for this questionâ€™s current Task/Answer.')] });
    ...
    async execute(interaction) {
        try {
            ...
            await botLog(interaction.guild, new Discord.EmbedBuilder()
                .setTitle('â›” Verification command failed')
