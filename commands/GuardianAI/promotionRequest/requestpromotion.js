...
module.exports = {
    nextTestQuestion: function(testData,promotion) {
        const category = bos[testData.section][testData.testType][testData.ind]
        console.log(category)
        console.log(promotion)
        // Object.keys(bos).forEach(category => {
            // bos[category][testTypes[promotion.requestor_nextRank]].forEach((i,index) => {
            //     if (i.display) {
            //         const ind = index + 1
            //         const question_num = `${testTypes[promotion.requestor_nextRank]} + ${ind}`
            //         console.log()
            //         const random_question_element = i.questions[Math.floor(Math.random() * i.questions.length)]
            //         const tester = { section: category, title: i.title, question: i.questions[random_question_element] }
            //         const grader = { section: category, title: i.title, question: i.questions[random_question_element], answer: i.questions[random_question_element] }

            //         console.log(random_question_element)
            //         console.log(tester)
            //         console.log(grader)
            //     }
            // })
    ...
    async execute(interaction) {
        ...
        if (interaction.options.getSubcommand() === 'knowledge_proficiency') {
                        ...
                        try {
                            ...
                            try {
                                ...
                                if (response.length > 0) {
                                    ...
                                    const values2 = [userId,leadershipSubmissionThread,requestorSubmissionThread]
                                    const sql2 = `
                                        INSERT INTO promotion (userId, leadership_threadId, requestor_threadId) 
                                        VALUES (?,?,?)   
                                        `
            ...
            // this.nextTestQuestion(testTypes,promotion)
            
            
            function testFunc(section,testType,ind) {
                const category = bos[section][testType][ind]
                console.log(category)
                if (category.display) {
                    const random_question_element = category.questions[Math.floor(Math.random() * category.questions.length)]
                    const tester = { section: capitalizeWords(category), title: category.title, question: category.questions[random_question_element] }
                    const grader = { section: capitalizeWords(category), title: category.title, question: category.questions[random_question_element], answer: category.questions[random_question_element] }
                    displayQuestion(question_number,tester,grader)
                }
            }
            takeTheTest()
            async function takeTheTest() {
                function quantities(section,testTypes,promotion) {
                    let quantities = {}
                    bos[section].forEach(i => {
                        quantities.push({
                            [section]: bos[section][testTypes[`${promotion.requestor_nextRank}_qty`]]
                        }) 
                    })
                    return quantities
                }
                //todo do database stuff to see where the last question was. Resuming functionality.

                let promotable_db_info = await checkPromotable()
                let question_embedId = null
                let inprogress_embedId = null
                let question = 0
                console.log(promotable_db_info)
                if (promotable_db_info[1].promotable) {
                    // function processArray(section,testType) {
                    //     const items = bos[section][testType]
                    ...
                    //     }
                    // }
                    function processArray(section,testType,currentIndex) {
                        const items = array[section][testType]
                    ...
                    if (promotable_db_info[0].question_num >= 1) {
                        ...
                        //todo get the current test question array index number
                        let currentIndex = currentIndex != 0 ? currentIndex = ind : currentIndex = 0
                        const question_position = processArray(section,testType,currentIndex)
                        console.log(question_position)
                        //todo save question_position as database entry question
                        //todo run next question in the array function (processArray) on button click.
                        //testFunc(section,testType,ind)
                        //test function combines data and then sends it to the appropriate threads in apply-for-ranks
                    }
                    //todo start test
                    else {
                ...
                async function displayQuestion(question_number,tester,grader) {
                    const quantities = quantities(section,testTypes,promotion)
                    //todo show embed
                    let inprogress_embedId = new Discord.EmbedBuilder()
                        .setTitle(`"${requestor_nextRank}" Promotion Knowledge Proficiency Test In-Progress`)
                        .setAuthor({ name: interaction.member.nickname, iconURL: interaction.user.displayAvatarURL({ dynamic: true }) })
                        .setThumbnail(botIdent().activeBot.icon)
                        .setColor('#87FF2A') //87FF2A
                        ...
                        .addFields(
                            { name: "Promotion Rank", value: "```" + requestor_nextRank + "```", inline: false },
                            { name: "Test Type", value: "```" + testTypes[requestor_nextRank] + "```", inline: false },
                            { name: "Section", value: "-", inline: true },
                            { name: "Questions", value: `${question_number}/${quantities[tester.section]}`, inline: true },
                            { name: "Total Questions", value: "-", inline: true }
                        )
                    let question_embedId = new Discord.EmbedBuilder()
                        .setTitle(`"${requestor_nextRank}" Promotion Knowledge Proficiency Test`)
                        .setAuthor({ name: interaction.member.nickname, iconURL: interaction.user.displayAvatarURL({ dynamic: true }) })
                        .setThumbnail(botIdent().activeBot.icon)
                        .setColor('#87FF2A') //87FF2A
                        .setDescription(`Submission of the Knowledge Proficiency test has been recorded.`)
                    ...
                    // question_embedId = await interaction.guild.channels.cache.get(staffChannel).send({ embeds: [question_embedId] })
                    // question_embedId = question_embedId.id
                    try {
                        // console.log(question_embedId)
